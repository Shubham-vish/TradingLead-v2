...
from SharedCode.Repository.CosmosDB.participants_data_repository import (
    ParticipantsDataCosmosRepository,
)
...
participants_data_container = kv_service.get_secret(
    Constants.PARTICIPANTS_DATA_CONTAINER
)
cosmos_db_service = CosmosDbService(database_id)
participants_data_cosmos_repo = ParticipantsDataCosmosRepository(
    cosmos_db_service, participants_data_container
)
    ...
    if is_weekday(date):
        ...
        if is_data_available(url):
            try:
                ...
                participants_data_cosmos_repo.store_data_for_day(
                    date, df, telemetry, tel_props
                )
                ...
                telemetry.info(
                    f"An error occurred while fetching data for {date.strftime('%d-%m-%Y')}: {e}"
                )
        else:
            telemetry.info(
                f"Data not available. Check why not available on this date: {date.strftime('%d-%m-%Y')}"
            )
    else:
