...
from SharedCode.Models.Fyers.orderbook_response import OrderBookResponse
...
class FyersService:
    ...
    def place_order(
        self, ticker_name: str, qty: int, product_type: str, side: OrderSide, tel_props
    ):
        ...
        for attempt in range(3):
            try:
                ...
                telemetry.info(
                    f"Placing {side_str} market response for {data}: {response}",
                    tel_props,
                )

                if response["s"] == Response.OK:
                    telemetry.info(
                        f"Placing {side_str} market response OK for {data}", tel_props
                    )
                    return response
                else:
                ...
                telemetry.exception(
                    f"Error in placing {side_str} market, attempt: {attempt}, error: {e}",
                    tel_props,
                )
                continue
    ...
    def place_buy_market(
        ...
        return self.place_order(
            ticker_name, qty, product_type, OrderSide.buy.value, tel_props
        )
    ...
    def place_sell_market(
        ...
        return self.place_order(
            ticker_name, qty, product_type, OrderSide.sell.value, tel_props
        )
        ...
        for attempt in range(3):
            try:
                ...
                if respnose["s"] == Response.OK:
                    telemetry.info(
                        f"Placing stoploss response OK for {data}", tel_props
                    )
                    return respnose
                else:
                ...
                telemetry.exception(
                    f"Error in placing stoploss, attempt: {attempt}, data: {data}, error : {e}",
                    tel_props,
                )
                continue
    ...
    def exit_positions(self, position_ids: List[str], tel_props):
        ...
        for attempt in range(3):
            try:
                ...
                if response["s"] == Response.OK:
                    telemetry.info(
                        f"Exiting positions response OK for {data}", tel_props
                    )
                    return response
                else:
                ...
                telemetry.exception(
                    f"Error in exiting positions, attempt: {attempt}, error: {e}",
                    tel_props,
                )
                continue
    ...
    def exit_all_positions(self, tel_props):
        ...
        for attempt in range(3):
            try:
                ...
                telemetry.exception(
                    f"Error in exiting all positions, attempt: {attempt}, error: {e}",
                    tel_props,
                )
                continue
    ...
    def get_positions(self, tel_props) -> NetPositionResponse:
        ...
        for attempt in range(3):
            try:
                ...
                telemetry.exception(
                    f"Error in getting positions, attempt: {attempt}, error: {e}",
                    tel_props,
                )
                continue
    ...
    def get_holdings(self, tel_props) -> HoldingsResponse:
        ...
        for attempt in range(3):
            try:
                ...
                telemetry.exception(
                    f"Error in getting holdings, attempt: {attempt}, error: {e}",
                    tel_props,
                )
                continue
    ...
    def get_order_book(self, tel_props) -> OrderBookResponse:
        ...
        for attempt in range(3):
            try:
                ...
                telemetry.exception(
                    f"Error in getting Order Book, attempt: {attempt}, error: {e}",
                    tel_props,
                )
                continue
    ...
    def cancel_orders(self, order_ids: List[str], tel_props):
        ...
        for attempt in range(3):
            try:
                ...
                if response["s"] == Response.OK:
                    telemetry.info(
                        f"Cancelled orders response OK for {data}", tel_props
                    )
                    return response
                else:
                ...
                telemetry.exception(
                    f"Error in cancelling orders, attempt {attempt}, error: {e}",
                    tel_props,
                )
                continue
    ...
    def set_stop_loss(self, stoploss: Stoploss, qty, tel_props):
        tel_props = tel_props.copy()
        tel_props.update(
            {"action": "setStoploss", "stoploss": asdict(stoploss), "qty": qty}
        )
    ...
    def set_stop_losses(self, stoplosses: List[Stoploss], tel_props):
        ...
        tel_props.update(
            {
                "action": "setStoplosses",
                "stoplosses": [asdict(stoploss) for stoploss in stoplosses],
                Constants.fyers_user_name: self.fyers_username,
                Constants.client_id: self.client_id,
            }
        )
        ...
        for stoploss in stoplosses:
            try:
                ...
                if not orderbook.is_same_stoploss_present(stoploss.ticker, qty):
                    cur_orders = orderbook.get_stoploss_orders_for_ticker(
                        stoploss.ticker
                    )
                    if cur_orders:
                        telemetry.info(
                            f"Current orders already exist for {stoploss.ticker}, {qty}: {cur_orders}",
                            tel_props,
                        )
                        self.cancel_orders(
                            [order.id for order in cur_orders], tel_props
                        )
                        telemetry.info(
                            f"Exited current orders for {stoploss.ticker}, {qty}: {cur_orders}",
                            tel_props,
                        )
                    else:
                        telemetry.info(
                            f"No current orders for {stoploss.ticker}, {qty}", tel_props
                        )
                    ...
                    telemetry.info(
                        f"Stoploss already set for -> {stoploss.ticker}, {qty}",
                        tel_props,
                    )
            except Exception as e:
        ...
        for order in orders:
            ...
            if order.order_type == OrderType.market.value:
                if order.side == OrderSide.buy.value.value:
                    ...
                    self.place_buy_market(
                        order.symbol, order.quantity, order.product_type, tel_props
                    )
    ...
    def is_stoploss_triggered(
        self,
        stoploss: Stoploss,
        hldng_res: HoldingsResponse,
        pos_res: NetPositionResponse,
        tel_props,
    ):
        tel_props = tel_props.copy()
        tel_props.update(
            {
                "action": "is_stoploss_triggered",
                "stoploss": asdict(stoploss),
                Constants.fyers_user_name: self.fyers_username,
                Constants.client_id: self.client_id,
            }
        )
    ...
    def get_quote(self, quote_req, tel_props) -> QuoteResponse:
        ...
        for attempt in range(3):
            try:
                ...
                telemetry.info(
                    f"Fetched quote response for {quote_req}: {response}", tel_props
                )
                quote_res = from_dict(data_class=QuoteResponse, data=response)
                return quote_res
            except Exception as e:
                telemetry.exception(
                    f"Error in fetching quote attempt: {attempt} for {quote_req}: {e}",
                    tel_props,
                )
                continue
