...
def TimerTriggerForAccessToken(
...
#     telemetry.info(
#         f"StopLoss Executor ServiceBus trigger function processing message: {message}",
#         tel_props,
#     )
...
#     telemetry.info(
#         f"StopLoss Executor ServiceBus parsed message properly: {json_message}",
#         tel_props,
#     )
...
#     telemetry.info(
#         f"StopLoss Executor ServiceBus trigger function processed message: {message}",
#         tel_props,
#     )
...
# -------------------------------
# @app.timer_trigger(
#     schedule="0 10,40 4,5,6,7,8,9,10 * * *",
#     arg_name="myTimer",
#     run_on_startup=False,
#     use_monitor=False,
# )
...
#     telemetry.info(
#         f"Timer trigger function stoploss_executor started at {utc_timestamp}",
#         tel_props,
#     )
...
#     telemetry.info(
#         f"set_stoplosses_for_all_users completed with results: {results}", tel_props
#     )
...
#     )
#     telemetry.info(
#         f"Timer trigger function stoploss_executor completed at {utc_timestamp}",
#         tel_props,
#     )
...
#     telemetry.info(
#         f"Timer trigger function MarketClosingExecutor started at {utc_timestamp}",
#         tel_props,
#     )
...
#     )
#     telemetry.info(
#         f"Timer trigger function MarketStartExecutor completed at {utc_timestamp} with results:{ results}",
#         tel_props,
#     )


# @app.timer_trigger(
#     schedule="0 40 4 * * *", arg_name="myTimer", run_on_startup=False, use_monitor=False
# )
...
#     telemetry.info(
#         f"Timer trigger function MarketStartExecutor started at {utc_timestamp}",
#         tel_props,
#     )
...
def Alerts(req: func.HttpRequest, context: func.Context) -> func.HttpResponse:
    ...
    if http_method == Constants.HTTP_DELETE:
        ...
        if not ticker:
            ...
            return func.HttpResponse(
                json_response, mimetype=application_json, status_code=400
            )
        ...
        tel_props.update({Constants.RESPONSE_BODY: json_response})
        return func.HttpResponse(
            json_response, mimetype="application/json", status_code=200
        )
    ...
    if http_method == Constants.HTTP_POST:
        ...
        user_id = req_body.get(
            Constants.REQUEST_PARAM_USER_ID
        )  # Replace this with actual user ID logic if needed
        ...
        if not trend_start or not trend_end or not ticker:
            json_response = json.dumps(
                {
                    "message": "Missing or empty value for trend_start, trend_end or ticker"
                }
            )
            tel_props.update({Constants.RESPONSE_BODY: json_response})
            telemetry.error(json_response, tel_props)
            return func.HttpResponse(
                json.dumps(json_response), mimetype=application_json, status_code=400
            )

        alerts_repo.create_user_alert(
            user_id, trend_start, trend_end, ticker, stock_name, telemetry, tel_props
        )
        ...
        tel_props.update({Constants.RESPONSE_BODY: json_response})
        telemetry.info(
            f"Returning Response after creating Alert for user: {user_id}", tel_props
        )
        return func.HttpResponse(
            json_response, mimetype="application/json", status_code=200
        )
