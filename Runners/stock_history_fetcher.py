...
def fetch_store_history_data_runner(tel_props):
    ...
    #         telemetry.info(
    #             f"fetching data for {ticker} from {sd} to {enddate}", tel_props
    #         )
    #         fyers_service.fetch_deep_history(
    #             ticker, sd, enddate, resolution="1", tel_props=tel_props
    #         )
    #         ticker = ticker.replace(":", "_")
    #         blob_name = f"{Constants.DIR_NIFTY_50}/{ticker}.csv"
    #         result = blob_service.create_blob(
    #             df, Constants.STOCK_HISTORY_CONTAINER, blob_name
    #         )
    #         telemetry.info(
    #             f"fetching data for {ticker} from {sd} to {enddate} completed, {result}",
    #             tel_props,
    #         )
    #     except Exception as e:
    #         telemetry.exception(
    #             f"fetching data for {ticker} from {sd} to {enddate} failed with exception: {e}",
    #             tel_props,
    #         )
