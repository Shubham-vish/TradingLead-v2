...
def check_and_execute_stoploss(
    stoploss: Stoploss,
    ...
    tel_props.update(
        {
            "action": "check_and_execute_stoploss",
            "stoploss": json.dumps(asdict(stoploss)),
            "user_id": user.id,
        }
    )
    ...
    if is_ltp_greater and curr_qty < stoploss.qty:
        telemetry.info(
            f"Buy Triggered for ticker: {stoploss.ticker} as ltp: {ltp} is greater than stoploss price: {stoploss.price} and cur_qty: {curr_qty} is less than stoploss qty: {stoploss.qty}",
            tel_props,
        )
        fyers_service.place_buy_market(
            stoploss.ticker, stoploss.qty - curr_qty, stoploss.product_type, tel_props
        )
    elif not is_ltp_greater and curr_qty > 0:
        telemetry.info(
            f"Sell triggered for ticker: {stoploss.ticker} as ltp: {ltp} is less than stoploss price: {stoploss.price} and cur_qty: {curr_qty} is more than 0",
            tel_props,
        )
        fyers_service.place_sell_market(
            stoploss.ticker, curr_qty, stoploss.product_type, tel_props
        )
    else:
        telemetry.info(
            f"Stoploss Sell/Buy not triggered for ticker: {stoploss.ticker} as ltp: {ltp} is greater than stoploss price: {stoploss.price} and cur_qty: {curr_qty} is greater than stoploss qty: {stoploss.qty}",
            tel_props,
        )


def execute_stoploss_for_user(
    user_stoplosses: UserStoplosses,
    check_at: StoplossCheckAt,
    tel_props,
):
    tel_props = tel_props.copy()
    tel_props.update(
        {
            "action": "execute_stoploss_for_user",
            "user_stoplosses": json.dumps(asdict(user_stoplosses)),
    ...
    if stoplosses is None or len(stoplosses) == 0:
        telemetry.info(
            f"No {check_at.value} stoplosses found for user: {user_stoplosses.user_id}",
            tel_props,
        )
    ...
    try:
        telemetry.info(
            f"Checking stoploss trigger for user: {user_stoplosses.id} : stoplosses: {asdict(stoplosses)}",
            tel_props,
        )

        user = user_repository.get_user(user_stoplosses.user_id, telemetry, tel_props)

        fyers_service = FyersService.from_kv_secret_name(
            user.kv_secret_name, kv_service
        )
        exceptions = []
        ...
        for stoploss in stoplosses:
            try:
                ltp = next(
                    (item for item in ticker_and_ltp if item.ticker == stoploss.ticker),
                    None,
                )
                curr_qty = holdings.get_quantity(
                    stoploss.ticker
                ) + positions.get_quantity(stoploss.ticker)
                check_and_execute_stoploss(
                    stoploss, fyers_service, user, curr_qty, ltp, tel_props
                )
            except Exception as e:
                telemetry.exception(
                    f"Error occurred while checking stoploss trigger for user: {user_stoplosses.id} : stoploss: {asdict(stoploss)}",
                    tel_props,
                )
                exceptions.append((e, asdict(stoploss)))
        telemetry.info(
            f"Stoploss set successfully for {asdict(user_stoplosses)}", tel_props
        )
        ...
        if len(exceptions) > 0:
            ...
            telemetry.error(
                f"Stoploss set successfully for {asdict(user_stoplosses)} but with exceptions: {exceptions}",
                tel_props,
            )
...
def execute_stoplosses_for_all_users(check_at: StoplossCheckAt, tel_props):
    ...
    telemetry.info(
        f"execute_stoplosses_for_all_users started with userStoplosses count: {len(users_stoplosses)}",
        tel_props,
    )
    results = []
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        futures = [
            executor.submit(
                execute_stoploss_for_user, user_stoplosses, check_at, tel_props
            )
            for user_stoplosses in users_stoplosses
        ]
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                if result:
                    ...
                    if success:
                        telemetry.info(
                            f"Stoploss execute successfully for user ID: {user_id}",
                            tel_props,
                        )
                    else:
                        telemetry.info(
                            f"Failed to execute stoploss for user ID: {user_id}",
                            tel_props,
                        )
                else:
...
def stoploss_executor_runner(check_at: StoplossCheckAt, tel_props):
    ...
    telemetry.info(
        f"stoploss_executor_runner completed with results: {results}", tel_props
    )
    return results
